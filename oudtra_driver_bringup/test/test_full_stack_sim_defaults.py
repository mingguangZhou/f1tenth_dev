...
SPIELBERG_DIRECTORY = Path(
    "centerline_tools/output_backup/"
    "V0_reward_ppo_speed_spielberg_1000k_20260612"
)
OPTIMIZED_RELATIVE = SPIELBERG_DIRECTORY / "raceline_points_optimized.csv"
MANUAL_RELATIVE = SPIELBERG_DIRECTORY / "raceline_points_smooth.csv"
REPORT_RELATIVE = (
    SPIELBERG_DIRECTORY / "raceline_points_optimized_validation.yaml"
)
...
def test_simulator_defaults_to_validated_optimized_raceline():
    ...
    assert defaults["centerline_csv_path"].endswith(
        "/centerline_points_smooth.csv"
    )

    optimized = REPOSITORY / OPTIMIZED_RELATIVE
    manual = REPOSITORY / MANUAL_RELATIVE
