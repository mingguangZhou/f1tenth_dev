...
def _load_yaml_parameters(config_path, node_name='raceline_publisher'):
...
def generate_launch_description():
    ...
    default_config = os.path.join(pkg_share, 'config', 'raceline_publisher_sim.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'frame_id',
            default_value='map',
            description='Frame id for published raceline',
        ),
        DeclareLaunchArgument(
            'path_topic',
            default_value='/raceline_path',
            description='Topic for nav_msgs/Path raceline',
        ),
        DeclareLaunchArgument(
            'marker_topic',
            default_value='/raceline_markers',
            description='Topic for RViz MarkerArray raceline visualization',
        ),
        DeclareLaunchArgument(
            'waypoints_topic',
            default_value='/raceline_waypoints',
            description='Topic for rich Float64MultiArray raceline waypoint rows',
        ),
        DeclareLaunchArgument(
            'direction',
            default_value='csv',
