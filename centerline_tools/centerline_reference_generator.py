...
import cv2
import matplotlib
if os.environ.get("CENTERLINE_HEADLESS", "0") == "1":
    matplotlib.use("Agg")
...
DRIVABLE_MASK_NPY_NAME = "drivable_region.npy"
...
MOVED_APEX_SAFETY_MARGIN_M = 0.26
MOVED_ENTRY_EXIT_SAFETY_MARGIN_M = 0.26
...
RACELINE_SAFETY_REGION_MARGIN_M = 0.25
...
RACELINE_OFFSET_GAUSSIAN_SIGMA_POINTS = 5.0
...
RACELINE_BSPLINE_SMOOTHING_FACTOR_PER_POINT = 0.00060 # defualt 0.00015
...
def main():
    ...
    # Extra Phase 9 artifact for the separated raceline generator.
    # This preserves the selected drivable region so Phase 10-12 do not need
    # to rebuild the skeleton/free-space pipeline.
    drivable_mask_path = os.path.join(OUTPUT_DIR, DRIVABLE_MASK_NPY_NAME)
    np.save(drivable_mask_path, free.astype(np.uint8))
    print(f"  Saved drivable mask:  {drivable_mask_path}")
